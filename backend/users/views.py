...
class Sign_up(APIView):
    def post(self, request):
        try:
            ...
            userimage = request.data.get('photo')  # Get the user image from the request data
        except KeyError:
        ...
        user = User.objects.create(
            ...
            phone=phone,
            password=password,
            userimage=userimage,  # Set the user image
        )
        user.save()
...
# perfectly working appoinment booking view   
...
#             print(f"Doctor Slot ID: {doctor_slot_id}, User ID: {user_id}")

#         except KeyError:
#             return Response({'error': 'Please provide doctor_slot and user in the request.'}, status=status.HTTP_400_BAD_REQUEST)

#         # Check if the provided user_id is valid
#         try:
#             doctor_slot = DoctorSlot.objects.get(id=doctor_slot_id)
#             user = User.objects.get(id=user_id)
#         except DoctorSlot.DoesNotExist:
#             return Response({'error': 'Doctor slot not found.'}, status=status.HTTP_404_NOT_FOUND)
#         except User.DoesNotExist:
...
#         if Appointment.objects.filter(doctor_slot=doctor_slot, user=user).exists():
#             return Response({"error": "User already has an appointment for this slot"}, status=status.HTTP_400_BAD_REQUEST)

#         # Check if the slot is available
#         if doctor_slot.is_available:
#             # Check if the user already has an appointment for the selected slot
#             if not Appointment.objects.filter(doctor_slot=doctor_slot, user=user).exists():
...
#                 # Serialize and return the appointment details
